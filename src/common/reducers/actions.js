  ...
  return async (dispatch, getState) => {
    ...
    try {
      ...
      try {
        ...
        console.error(err);
        throw new Error(`Invalid username or password.${err}`);
      }

      if (!data?.selectedProfile?.id) {
        throw new Error("It looks like you didn't buy the game.");
      }
      const idforcape = password;
      try {
        const skinUrl = await getPlayerSkin(idforcape);
        if (skinUrl) {
          data.skin = skinUrl;
          console.log(`Skin found: ${skinUrl}`);
        }
      } catch (err) {
        console.error(err);
        throw new Error('Invalid UID for skin');
      }
      data.selectedProfile.id = idforcape;
